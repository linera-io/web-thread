...
impl Thread {
        ...
        std::thread::spawn(|| {
            ...
            executor.run_until(async move {
                while let Some(task) = receiver.next().await {
                    spawner.spawn_local_obj(task()).expect("executor should exist until destroyed");
                }
            });
        });
        Ok(Self { sender })
    ...
    pub fn run<Context: Post, F: Future<Output: Post> + 'static>(
        ...
        self.sender.unbounded_send(Box::new(move || Box::new(async move {
            let _ = sender.send(code(context).await);
        }).into())).unwrap_or_else(|_| panic!("worker shouldn't die unless dropped"));
        async move {
            Ok(receiver.await.unwrap_or_else(|_| panic!("worker shouldn't die unless dropped")))
        }
    }
}
...
fn basic_functionality() {
    assert_eq!(
        ...
        futures::executor::LocalPool::new()
            .run_until(Thread::new().unwrap().run(3u8, |three| async move { three + 5 }))
            .unwrap(),
    );
}
